##### Serveur - Partie dynamique des visuels =================================
# 
# Dans cette partie nous définissons la logique dynamique du coté Serveur.
# Les visuels (boutons, champs, widgets) n'adaptent en fonction du choix de user.
# Par conséquent ces derniers réagissent à des évènements déclencher par l'user
# Pour faire apparaître les possibilités qui s'offrent à lui dans son expérience.
# 
...
#       séparés dépendant du rôle de chacuns et de l'objectif poursuivis.
#       
#       Ces fichiers sont importer via la commande <source> 
#       
##### ================================= *** =================================
...
server = function (input, output, session) {
      ...
      if(input$input_format == "csv"){
        ...
        try({
          ...
          output$input_process = renderUI({
            actionButton(
              "input_process",
              label = "Faire le processing du DataFrame final")
          })
  ...
  observeEvent(input$input_process, {
    ...
    if (number_click_process == 1){
      list_reavalues$table_all = process(
        list_reavalues$table_all,
        list_reavalues$col_categories_all)
    }
    list_reavalues$table = process(
      list_reavalues$table,
      list_reavalues$col_categories)
    list_reavalues$type_table = data.frame(
      variable = colnames(list_reavalues$table),
  ...
  observeEvent(input$input_valid_exp,{
    if (input$input_exp_type == "Univariée") {
      ...
      if (viz_type == "Tableau des Effectifs/Fréquence") {
        type_col = get_type_col_by_name(
          list_reavalues$type_table,
          nput$input_uni_select_var)
        ...
        output$viz_input = renderUI({
          renderTable(
            create_freq_table_continous(
              df_col,nbins = as.integer(input$bins_input)))
        })
        ...
        output$viz_input = renderUI({
          renderPlot({ 
            create_histogram_continous(
              df_col,
              nbins = as.integer(input$bins_input))
            })
        })
      } else if (viz_type == "Tableau des Effectifs/Fréquence par classe") {
        output$viz_input = renderUI({
    ...
    if (input$input_exp_type == "Bivariée" && 
      ...
      type_col1 =
        list_reavalues$col_categories[match(input$input_bi_select_var[1], 
                                            names(list_reavalues$col_categories))]
      type_col2 = list_reavalues$col_categories[
      ...
      if (viz_type == "Statistiques"){
        table = two_var_statistics(
          df_cols,type_cols = c(type_col1,type_col2),
          nbins=as.integer(input$bins_input))
        ...
        table = two_var_contingency_table(
          df_cols,type_cols = c(type_col1,type_col2), 
          nbins = as.integer(input$bins_input))
    ...
    if (!length(features)){
      ...
      output$input_model_features_bin=renderUI({
        selectizeInput(
          "input_model_features_bin",
          label="Selectionner vos features",
          choices=features,multiple=TRUE,
          options = list(maxItems = length(features)),
          selected =features[1])
      })
    }
  ...
  observeEvent(input$input_valid_model_bin,{
    ...
    metrics_confusion= 
      calculate_metrics(true_labels_test,pred_prob_test>threshold)
  ...
  output$output_table = renderDataTable({
    if(!is.null(list_reavalues$table)) 
      DT::datatable(list_reavalues$table,
  ...
  output$output_type_table = renderDataTable({
    if(!is.null(list_reavalues$type_table)) 
      DT::datatable(list_reavalues$type_table,
                    options = list(scrollX=TRUE,
                                   scrollY = "200px"))
  })
